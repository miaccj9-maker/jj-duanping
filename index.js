...
async function renderCard() {
    ...
    // 拍立得模板：强制硬编码 CSS
    const POLAROID_CSS = `.be-card.be-custom{background:#fafafa;padding:14px 14px 90px 14px;box-shadow:0 4px 16px rgba(0,0,0,.18);position:relative;width:340px;box-sizing:border-box;font-family:-apple-system,"PingFang SC","Microsoft YaHei",sans-serif;}.be-card.be-custom .be-char-avatar{display:none!important;}.be-card.be-custom .be-source .author{display:none!important;}.be-card.be-custom .be-source .chapter{display:none!important;}.be-card.be-custom .be-quote-orig{display:none!important;}.be-card.be-custom .be-watermark{display:none!important;}.be-card.be-custom .be-comment{display:none!important;}.be-card.be-custom .be-date-cn{display:none!important;}.be-card.be-custom .be-head{position:absolute;bottom:110px;right:22px;z-index:3;text-align:right;pointer-events:none;max-width:64%;white-space:nowrap;}.be-card.be-custom .be-name{font-size:16px;color:rgba(0,0,0,.62);display:block!important;cursor:pointer;pointer-events:auto;white-space:nowrap;}.be-card.be-custom .be-date{font-size:14px;color:rgba(0,0,0,.45);display:block!important;cursor:pointer;pointer-events:auto;white-space:nowrap;}.be-card.be-custom .be-quote{display:block;margin:0;padding:12px 10px 16px 10px;background:#e9e9e7;color:#2b2b2b;font-size:15px;line-height:1.65;min-height:280px;box-sizing:border-box;white-space:pre-wrap;word-break:break-word;position:relative;}.be-card.be-custom .be-source{position:absolute;bottom:0;left:0;right:0;height:90px;display:flex;align-items:flex-end;justify-content:center;padding:0 12% 24px 12%;box-sizing:border-box;}.be-card.be-custom .be-source .title{font-size:28px;color:#2a2a2a;font-weight:700;cursor:pointer;text-align:center;width:100%;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}`;
    ...
    if (isPolaroid) {
        ...
        let titleSize = 28;
        if (len > 14) titleSize = 26;
        if (len > 18) titleSize = 22;
        if (len > 23) titleSize = 18;
        if (len > 29) titleSize = 14;
        let srcH = Math.max(56, Math.round(((cardH > 0 ? cardH : 280) + 14) / 3));
        let nameSize = 16;
        let dateSize = 14;
        const wRatio = cardW > 0 ? (cardW / 340) : 1;
        // 综合缩放：宽度变宽字号变大、变窄变小；高度(卡片总高)变低时同步缩小，取较紧一维避免溢出
        const baseTotalH = 392; // 340宽+默认正文280：总高=14+280+98，底部标题框占25%
        const totalH = Math.round(((cardH > 0 ? cardH : 280) + 14) * 4 / 3);
