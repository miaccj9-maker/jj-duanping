...
async function renderCard() {
    ...
    // 拍立得模板：强制硬编码 CSS
    const POLAROID_CSS = `.be-card.be-custom{background:#fafafa;padding:16px 16px 64px 16px;box-shadow:0 4px 16px rgba(0,0,0,.18);position:relative;width:340px;box-sizing:border-box;font-family:-apple-system,"PingFang SC","Microsoft YaHei",sans-serif;}.be-card.be-custom .be-char-avatar{display:none!important;}.be-card.be-custom .be-source .author{display:none!important;}.be-card.be-custom .be-source .chapter{display:none!important;}.be-card.be-custom .be-quote-orig{display:none!important;}.be-card.be-custom .be-watermark{display:none!important;}.be-card.be-custom .be-comment{display:none!important;}.be-card.be-custom .be-date-cn{display:none!important;}.be-card.be-custom .be-head{position:absolute;bottom:76px;right:22px;z-index:3;text-align:right;pointer-events:none;max-width:64%;white-space:nowrap;}.be-card.be-custom .be-name{font-size:15px;color:rgba(0,0,0,.62);display:block!important;cursor:pointer;pointer-events:auto;white-space:nowrap;}.be-card.be-custom .be-date{font-size:15px;color:rgba(0,0,0,.45);display:block!important;cursor:pointer;pointer-events:auto;white-space:nowrap;}.be-card.be-custom .be-quote{display:block;margin:0;padding:12px 10px 16px 10px;background:#e9e9e7;color:#2b2b2b;font-size:15px;line-height:1.65;min-height:280px;box-sizing:border-box;white-space:pre-wrap;word-break:break-word;position:relative;}.be-card.be-custom .be-source{position:absolute;bottom:0;left:0;right:0;height:64px;display:flex;align-items:center;justify-content:center;padding:0 2%;box-sizing:border-box;}.be-card.be-custom .be-source .title{font-size:32px;color:#2a2a2a;font-weight:700;cursor:pointer;text-align:center;width:100%;line-height:1.2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}`;
    ...
    if (isPolaroid) {
        ...
        let srcH = 64; // 底部白边固定64px，不随高度滑块变化
        ...
        const baseTotalH = 360; // 340宽+默认正文280：总高=16+280+64
        const totalH = ((cardH > 0 ? cardH : 280) + 16) + 64;
        const ratio = wRatio; // 字号宽度主导：变宽变大、变窄变小，不被高度拖累
        // 单行自适应：能容纳则占白边高70%，文字多则缩至一行能放下（不换行）
        titleSize = Math.min(Math.round(srcH * 0.85), Math.max(Math.round(srcH * 0.7), Math.floor(((cardW > 0 ? cardW : 340) - 32) * 0.96 / len)));
        // 单行模式：无迭代换行
...
(function(){var _els=[document.querySelector('.be-card.be-custom.polaroid-card .be-name'),document.querySelector('.be-card.be-custom.polaroid-card .be-date')];for(var _e=0;_e<_els.length;_e++){var _pt=_els[_e];if(!_pt)continue;var _fs=parseFloat(getComputedStyle(_pt).fontSize)||18;for(var _i=0;_i<60;_i++){if(_pt.scrollWidth<=_pt.clientWidth+1||_fs<=8)break;_fs-=1;_pt.style.setProperty('font-size',_fs+'px','important');}}})();
</script>
</body>
</html>`;
