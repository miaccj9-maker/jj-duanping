...
.be-card.be-custom .be-quote {
  ...
  line-height: var(--be-quote-lh, 1.5) !important;
...
.dp-collage-card {
  position: relative;
  overflow: visible; /* 拼贴词条可突破卡片边界显示（仍可用内联 overflow 覆盖） */
  box-sizing: border-box;
}
.dp-collage-word {
  position: absolute;
...
/** 存档用的角色名：优先 getContext 当前角色，兜底旧全局 char */
function safeCharName() {
    try {
        const n = getCurrentCharacter().name;
        if (n && n !== '角色') return n;
    } catch (e) {}
    try {
        if (typeof char !== 'undefined' && char && char.name) return char.name;
    } catch (e) {}
    return '';
}
...
async function renderCard() {
    ...
    if (collageActive) {
        ...
        if (settings.bgImage) {
            ...
            collageCardHtml = `<div class="be-card be-custom dp-collage-card" style="width:${w0}px;height:${h0}px;background-color:${settings.collageBgColor || '#f5eedd'};position:relative;overflow:visible;box-sizing:border-box;">${inner}</div>`;
        }
    }
    let html;
    ...
    html = (collageCardHtml ? collageCardHtml + stickersHtml : null) || (buildCardHtml({
        ...
        quoteLh: Number(settings.quoteLh) || 1.5,
    ...
    if (isPolaroidTpl && !collageCardHtml) {
        ...
        const polaroidComments = commentsInside ? commentsHtml : '';
        const polaroidStickers = stickersHtml || '';
        html = `<div class="dp-card-wrap"><div class="be-card be-custom polaroid-card" style="--be-quote-size:${Number(settings.quoteSize)||15}px;--be-quote-lh:${Number(settings.quoteLh)||1.6};--be-quote-ls:${Number(settings.quoteLs)||0}em">
          <div class="be-quote">${esc(polaroidQuote)}</div>
          <div class="be-head"><span class="be-date">${esc(polaroidDate)}</span><span class="be-name">${esc(polaroidName)}</span></div>
          <div class="be-source"><span class="title">${esc(polaroidTitle)}</span></div>
          ${polaroidComments}
          ${polaroidStickers}
        </div></div>`;
    }
    ...
    // 拍立得模板：强制硬编码 CSS
    const POLAROID_CSS = `.be-card.be-custom{background:#fafafa;padding:16px 16px 165px 16px;box-shadow:0 4px 16px rgba(0,0,0,.18);position:relative;width:340px;box-sizing:border-box;font-family:-apple-system,"PingFang SC","Microsoft YaHei",sans-serif;}.be-card.be-custom .be-char-avatar{display:none!important;}.be-card.be-custom .be-source .author{display:none!important;}.be-card.be-custom .be-source .chapter{display:none!important;}.be-card.be-custom .be-quote-orig{display:none!important;}.be-card.be-custom .be-watermark{display:none!important;}.be-card.be-custom .be-comment{display:none!important;}.be-card.be-custom .be-date-cn{display:none!important;}.be-card.be-custom .be-head{position:absolute;bottom:177px;right:22px;z-index:3;text-align:right;pointer-events:none;max-width:64%;white-space:nowrap;}.be-card.be-custom .be-name{font-size:15px;color:rgba(0,0,0,.62);display:block!important;cursor:pointer;pointer-events:auto;white-space:nowrap;}.be-card.be-custom .be-date{font-size:15px;color:rgba(0,0,0,.45);display:block!important;cursor:pointer;pointer-events:auto;white-space:nowrap;}.be-card.be-custom .be-quote{display:block;margin:0;padding:12px 10px 16px 10px;background:#e9e9e7;color:#2b2b2b;font-size:15px;line-height:1.65;min-height:280px;box-sizing:border-box;white-space:pre-wrap;word-break:break-word;position:relative;}.be-card.be-custom .be-source{position:absolute;bottom:0;left:0;right:0;height:165px;display:flex;align-items:center;justify-content:center;padding:0 10%;box-sizing:border-box;}.be-card.be-custom .be-source .title{font-size:32px;color:#2a2a2a;font-weight:700;cursor:pointer;text-align:center;width:100%;line-height:1.15;white-space:normal;word-break:break-all;}`;
    ...
    const heightCss = (cardH > 0)
        ? `.be-card.be-custom{height:auto!important}.be-card.be-custom .be-quote{height:${cardH}px!important;min-height:${cardH}px!important;overflow:hidden!important}`
        : '';
    ...
    if (isPolaroid) {
        ...
        let srcH = 165; // 底部白边固定165px，不随高度滑块变化
        ...
        const baseTotalH = 461; // 340宽+默认正文280：总高=16+280+165
        const totalH = ((cardH > 0 ? cardH : 280) + 16) + 165;
        const ratio = wRatio; // 字号宽度主导：变宽变大、变窄变小，不被高度拖累
        titleSize = Math.max(14, Math.round(titleSize * ratio));
        // 迭代换行：标题文字块总高占满白边70-80%，不省略不溢出
        for (let _ti = 0; _ti < 50; _ti++) { const _perLine = Math.max(1, Math.floor((cardW > 0 ? cardW : 340) * 0.8 / titleSize)); const _lines = Math.ceil(len / _perLine); const _tot = _lines * titleSize * 1.15; if (_tot <= srcH * 0.85 || titleSize <= 12) break; titleSize -= 2; }
        nameSize = Math.max(10, Math.round(nameSize * ratio));
        dateSize = Math.max(9, Math.round(dateSize * ratio));
        polaroidTitleCss = '.be-card.be-custom{padding-bottom:' + srcH + 'px!important}.be-card.be-custom .be-source{height:' + srcH + 'px!important}.be-card.be-custom .be-head{bottom:' + Math.round(srcH + 12) + 'px!important}.be-card.be-custom .be-source .title{font-size:' + titleSize + 'px!important;}.be-card.be-custom .be-name{font-size:' + nameSize + 'px!important;}.be-card.be-custom .be-date{font-size:' + dateSize + 'px!important;}';
    }
...
<style>${finalCss}</style>${isPolaroidTpl ? `<style>${POLAROID_CSS}</style>` : ``}
...
function showFloatButton(rect) {
    ...
    if (!btn) return;
    // 先显示再测量：display:none 时 offsetWidth/offsetHeight 为 0，会导致两个半区之间出现空隙
    ...
    let left, top;
    if (isMobile) {
        // 手机端：一体化胶囊按钮放选区右侧（段评在左、存档在右）
        left = rect.right + 10;
        top = rect.top + rect.height / 2 - h / 2;
    } else {
        // 电脑端：一体化胶囊按钮放选区右侧（段评在左、存档在右）
        left = rect.right + 10;
        top = rect.top + rect.height / 2 - h / 2;
        if (left + w + sw > window.innerWidth - 8) {
            // 放不下就整体放左侧，保持段评在左、存档在右
            left = rect.left - w - sw - 16;
        }
    }
    // 合并后：整体钳制在可视区内，避免选区贴近屏幕边缘时按钮溢出屏幕（手机端放不下时贴屏幕右缘，不跳左侧）
    const totalW = w + sw;
    const maxLeft = Math.max(8, window.innerWidth - totalW - 8);
    left = Math.min(Math.max(8, left), maxLeft);
    top = Math.min(Math.max(8, top), window.innerHeight - h - 8);
    btn.style.left = left + 'px';
    if (saveBtn) saveBtn.style.left = (left + w) + 'px';
    btn.style.top = top + 'px';
    if (saveBtn) saveBtn.style.top = top + 'px';
    // 最终保险：按视觉视口实测校准。页面被缩放（pinch zoom）时 fixed 元素相对布局视口定位，视觉上可能偏出屏幕，
    // 这里读取按钮真实渲染坐标（相对视觉视口）并把它拉回可视区内，不依赖 innerWidth 假设。
    try {
        const vis = window.visualViewport;
        if (vis) {
            const zoom = vis.scale || 1;
            const visW = vis.width;
            const br2 = btn.getBoundingClientRect();
            const sr2 = saveBtn ? saveBtn.getBoundingClientRect() : null;
            const curL = br2.left;
            const curR = (sr2 ? sr2.right : br2.right);
            const needLeftShift = (curR - (visW - 8)) * zoom;   // >0 表示右缘超界，需要左移
            const needRightShift = (8 - curL) * zoom;            // >0 表示左缘超界，需要右移
            if (needLeftShift > 0) {
                left = Math.max(8, left - needLeftShift);
                btn.style.left = left + 'px';
                if (saveBtn) saveBtn.style.left = (left + w) + 'px';
            } else if (needRightShift > 0) {
                left = left + needRightShift;
                btn.style.left = left + 'px';
                if (saveBtn) saveBtn.style.left = (left + w) + 'px';
            }
        }
...
function injectShell() {
    ...
    link.rel = 'stylesheet';
    link.href = `${extensionFolderPath}/style.css?v=1789713332`;
...
`;
    ...
    // 滚动/旋转/窗口变化时：若浮动胶囊仍显示，跟随选区重新定位，避免按钮脱离选区或滚出屏幕
    let _dpReposTimer = null;
    const repositionFloat = () => {
        const fb = document.getElementById('dp-float-btn');
        if (!fb || fb.style.display === 'none') return;
        if (_dpReposTimer) return;
        _dpReposTimer = setTimeout(() => {
            _dpReposTimer = null;
            try { onSelectionChange(); } catch (e) { /* noop */ }
        }, 60);
    };
    document.addEventListener('scroll', repositionFloat, true);
    window.addEventListener('resize', repositionFloat);
    window.addEventListener('orientationchange', repositionFloat);
